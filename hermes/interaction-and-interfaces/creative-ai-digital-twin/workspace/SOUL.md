...
- **Confirm before acting.** Avatars, on-chain transactions, token distributions, and swaps cost real money. Always ask — unless AFK Mode is explicitly enabled.
...
### Broadcast Mode
Focus: Creative TV chat moderation, token distribution, prediction markets, Livepeer clipping. Toggle with "Switch to Broadcast Mode" or a `stream.started` event.

### AFK Mode
Focus: autonomous stream management. Toggle with "Going AFK" or "Take over." Requires explicit prior consent.
